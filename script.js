        ...
        authForm.addEventListener('submit', (e) => {
            ...
            if (isLoginMode) {
                ...
                if (user) {
                    ...
                    authContainer.style.display = 'none';
                    appContainer.style.display = 'flex';
                    // We don't reset the form here so the browser can prompt to save the password
                } else {
                    const existingUser = users.find(u => u.email === email);
                    if (existingUser) {
                        alert('Invalid email or password!');
                    } else {
                        // User not found in this device's local storage. Auto-register so they don't have to sign up again on a new device.
    ...
    if (logoutBtn) {
        logoutBtn.addEventListener('click', () => {
            if (confirm('Are you sure you want to log out?')) {
                ...
                appContainer.style.display = 'none';
                authContainer.style.display = 'flex';
                authForm.reset(); // Clear the form on logout instead
